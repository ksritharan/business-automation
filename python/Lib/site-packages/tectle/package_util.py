...
def package_new_receipts(cur, new_receipt_ids, receipt_items):
    ...
    for row in rows:
        ...
        sku_list = list(receipt_items[receipt_id])
        # NO ACCESSORIES IN PACKAGES
        sku_list = [sku for sku in sku_list if sku.strip().startswith('P')]
...
def package_new_receipt(cur, receipt_id, cost_key, sku_list, package_configs):
    query = """
        SELECT p.package_class_id,
               ...
               r.country
          FROM receipts r
          JOIN printer_queue pq ON r.receipt_id = pq.receipt_id
...
def get_packaging(cur, product_order, package_configs, class_sku_lists, cost_key):
    ...
    del best_comb['cost']
    packaging_data = []
    logger.debug('in get_packaging()')
    logger.debug('Product Order: %s' % str(product_order))
    logger.debug('Class Sku Lists: %s' % str(class_sku_lists))
    logger.debug('Best Package Combination: %s' % str(best_comb))
    logger.debug('Package Configs: %s' % str(package_configs)
