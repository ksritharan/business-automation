...
def get_webflow_orders():
    conn = get_connection()
    cur = conn.cursor()
    num_receipts = 1
    receipts = []
    while num_receipts > 0:
        new_receipts = update_webflow_orders_helper(len(receipts))
        receipts.extend(new_receipts)
        num_receipts = len(new_receipts)
    queue_items, new_sku_lists = add_new_webflow_receipts(cur, receipts)
    add_webflow_to_printer_queue(cur, queue_items)
    new_receipt_ids = list({item['receipt_id'] for item in queue_items})
    package_new_receipts(cur, new_receipt_ids, new_sku_lists)
    conn.commit()
    

def update_webflow_orders_helper(offset):
    url = None
    headers = {}
...
def add_new_webflow_receipts(cur, receipts):
    receipt_ids = [str(receipt['orderId']) for receipt in receipts]
    query = """
        SELECT receipt_id
          FROM receipts
    ...
    sku_lists = {}
    for receipt in receipts:
        if receipt['orderId'] not in old_receipts:
            data = {
            ...
            new_receipts.append(data)
            sku_list = []
            for purchased_items in receipt['purchasedItems']:
                skus = purchased_items['variantSKU'].split('.')
                for i in range(len(skus)):
                    sku = skus[i].strip()
                    for i in range(purchased_items['count']):
                        queue_item = {'receipt_id': receipt['orderId'], 'sku': sku}
                        queue_items.append(queue_item)
                for i in range(purchased_items['count']):
                    sku_list.append(purchased_items['variantSKU'])
            sku_lists[receipt['orderId']] = sku_list
                
    values = ',\n'.join(["('%(name)s', '%(receipt_id)s', '%(first_line)s', '%(second_line)s', '%(city)s', '%(state)s', '%(zip)s', '%(country)s', '%(country_id)s', '%(buyer_email)s', 'Webflow')" % receipt
                for receipt in new_receipts])
    if new_receipts:
        query = """
            INSERT INTO receipts (name, receipt_id, first_line, second_line, city, state, zip, country, country_id, buyer_email, source)
            VALUES %s
        """ % values
        cur.execute(query)
    return queue_items, sku_lists
    
def add_webflow_to_printer_queue(cur, queue_items):
...
def add_new_receipts(cur, receipts):
    ...
    values = ',\n'.join(["('%(name)s', '%(receipt_id)s', '%(first_line)s', '%(second_line)s', '%(city)s', '%(state)s', '%(zip)s', '%(country)s', '%(country_id)s', '%(buyer_email)s', 'Etsy')" % receipt
                for receipt in new_receipts])
    if new_receipts:
        query = """
            INSERT INTO receipts (name, receipt_id, first_line, second_line, city, state, zip, country, country_id, buyer_email, source)
...
def complete_order_webflow(receipt_id, tracking_pin):
