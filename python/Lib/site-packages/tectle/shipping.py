...
def create_shipping_label_helper(cur, receipt_id):
    ...
    for package_id in packages:
        ...
        context = {
            ...
            'xpresspost': packages[package_id]['state'] in ('ON', 'QC')
        }
...
def create_merged_file(cur, receipt_id):
    ...
    abs_filename = combine_files(receipt_id, files, 'receipt')
...
def create_manifest(cur, group_id):
    try:
        ...
        filename = combine_files(group_id, filenames, 'manifest')
        query = """
            UPDATE manifests
               SET filename = '%s'
             WHERE group_id = '%s'
...
def combine_files(tag, files, prefix):
    ...
    filename2 = 'static/pdfs/%s %s %s_merged.pdf' % (prefix, tag, datetime.today().strftime('%Y-%m-%d'))
    ...
    load_config()
    try:
        if 'PDF_FOLDER' in session and prefix == 'receipt':
        ...
        elif 'MAN_FOLDER' in session and prefix == 'manifest':
            pdf_folder = session['MAN_FOLDER']
            basename = os.path.basename(filename2)
            new_path = os.path.join(pdf_folder, basename)
            copyfile(filename2, new_path)
    except Exception as e:
...
def scale_and_combine(receipt_id, files):
    output = PdfFileWriter()
    for filename in files:
        existing_pdf = PdfFileReader(open(filename, "rb"))
        page = existing_pdf.getPage(0)
        page.scaleTo(4*inch, 6*inch)
        output.addPage(page)
