...
def create_merged_file(cur, receipt_id):
    ...
    abs_filename = combine_files(receipt_id, files, prefix='receipt')
...
def create_manifest(cur, group_id):
    try:
        ...
        filename = combine_files(group_id, filenames, prefix='manifest')
        query = """
            UPDATE manifests
               SET filename = '%s'
             WHERE group_id = '%s'
...
def combine_files(tag, files, prefix=None):
