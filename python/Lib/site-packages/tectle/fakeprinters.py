...
def status_templates(status_type='1', **override):
    ...
    if status_type == '1':
        ...
        keys = ['status', 'currentLayer', 'printDuration', 'timesLeft']
        for key in keys:
            if key in override:
                data[key] = override[key]
        response = dumps(data)
...
def do_printer_upload(filename, time, request):
    logger.debug("incoming upload request")
...
def do_printer_fileinfo(printer_ip):
    conn = get_connection()
    cur = conn.cursor()
    printer = get_printer_info(cur, printer_ip)
    file_info = {"err":0,"size":457574,"height":4.00,"layerHeight":0.25,"filament":[6556.3],"generatedBy":"Slic3r 1.1.7 on 2014-11-09 at 17:11:32"}
    ...
    if printer['status'] == PRINTING:
        if printer['current_time'] > printer['finish_time']:
            file_info['fractionPrinted'] = 0
            cur.execute("UPDATE FAKE_PRINTERS SET status = '0' WHERE printer_ip = '%s'" % printer_ip)
            conn.commit()
        else:
            fraction_printed = round(((printer['current_time']-printer['start_time'])/(printer['finish_time']-printer['start_time'])), 3)
            file_info['fractionPrinted'] = fraction_printed
    last_resp = dumps(file_info)

    return make_response(last_resp, 200)