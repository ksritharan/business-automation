    ...
    if not printers:
        ...
        try:
            ...
            if not printer_queues:
                ...
                make_print_request(conn, cur, printer['printer_ip'], white_sku)
    ...
    if response_json['status'] != 'I': #idle
        ...
        if not file_exists:
            ...
            if not is_up_to_date:
                success = send_file_to_printer(printer_url, gcode_filename)
                if not success:
...
def send_file_to_printer(printer_url, gcode_filename):
    fullpath = os.path.join(session['GCODE_FOLDER'], gcode_filename)
    data = None
    with open(fullpath, 'rb') as fh:
        data = fh.read()
    response = requests.post(printer_url + ('rr_upload?name=%s' % gcode_filename), data=data)
...
def update_printer(conn, cur, printer, is_printer_debug):
    ...
    try:
        ...
        r = requests.get(printer_url + 'rr_status')
        r_json = r.json()
    except Exception as e:
        logging.exception(e)
        ...
        try:
            if r_json['status'] == 'I' and printer['str_status'] == 'printing':
                ...
                if not printer['gcode']:
                    m36_response = requests.get(printer_url + 'rr_fileinfo')
                    m36_response.raise_for_status()
                    m36_response_json = m36_response.json()
                    update_gcode(cur, printer['printer_id'], os.path.basename(m36_response_json['fileName']))
                # update progress
