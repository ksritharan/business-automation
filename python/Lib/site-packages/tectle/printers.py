    ...
    if not printers:
        ...
        try:
            ...
            if not printer_queues:
                ...
                make_print_request(printer['printer_ip'], next_print['sku'])
                conn = get_connection()
                cur = conn.cursor()
...
def make_print_request(printer_ip, sku):
    ...
    response = requests.get(printer_url + 'rr_gcode?gcode=M408 S0')
...
def update_printer(conn, cur, printer, is_printer_debug):
    ...
    try:
        ...
        r = requests.get(printer_url + 'rr_gcode?gcode=M408 S0')
        r.raise_for_status()
        r_json = r.json()
    except Exception as e:
        logging.exception(e)
                ...
                query = """
                    SELECT COUNT(1)
                     ...
                     WHERE status <> 2 /* COMPLETED */
                       AND receipt_id = '%s'
                """ % (printer['current_receipt'],)
                cur.execute(query)
                is_complete = cur.fetchone()[0] == 0
                if is_complete:
                    query = """
                        UPDATE receipts
                           SET status = 1 /* PRINTED */
                         WHERE receipt_id = '%s'
                    """ % (printer['current_receipt'],)
                    cur.execute(query)
...
def update_gcode(cur, printer, gcode):
    query = """
        UPDATE printers
           SET gcode = '%s'
         WHERE id = %s
