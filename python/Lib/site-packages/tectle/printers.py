...
from .file_manager import update_local_files, is_printer_up_to_date, send_file_to_printer
import requests
import logging
...
def make_print_request(conn, cur, printer_ip, sku):
    ...
    if response_json['status'] != 'I': #idle
        ...
        update_local_files(conn, cur, gcode_folder, 'gcodes')
        conn.commit()
        file_exists = gcode_file_exists(cur, gcode_filename)
        if not file_exists:
            raise Exception('%s could not be found' % gcode_filename)
        else:
            is_up_to_date = is_printer_up_to_date(cur, printer_ip, gcode_filename, 'gcodes')
            if not is_up_to_date:
                fullpath = os.path.join(gcode_folder, gcode_filename)
                callback_fn_args = (printer_ip, gcode_filename)
                success = send_file_to_printer(printer_url, fullpath, 'gcodes', callback_fn=update_percent, callback_fn_args=callback_fn_args)
                if not success:
                    raise Exception('error sending %s to printer %s' % (gcode_filename, printer_ip))
                else:
                    update_percent(100, printer_ip, gcode_filename)
...
def update_percent(percent, printer_ip, filename):
    conn = get_connection()
    cur = conn.cursor()
    data = {
    ...
    if r_json is not None:
        try:
            if r_json['status'] == 'I' and printer['str_status'] == 'printing':
                ...
                # update progress
                progress = int(r_json['fractionPrinted'])
                query = """
                    UPDATE PRINTER_QUEUE
                       SET progress = %s
                     WHERE printer_id = %s
...
def get_next_in_unassigned_queue(color, waterplate_only=0):
    ...
    query1 = """
        SELECT sku
         ...
         WHERE color = '%(color)s'
           AND ( 
                  (0 = %(waterplate_only)s AND category = 'Planter')
               OR (1 = %(waterplate_only)s AND name = 'WaterPlate')
               )
