...
class Etsy(Source):
    ...
    def complete_order(self, receipt_id, tracking_pin):
        data = {'tracking_pin': tracking_pin, 'carrier_name': 'canada-post'}
        url = 'https://openapi.etsy.com/v2/shops/%s/receipts/%s/tracking' % (session['SHOP_ID'], receipt_id)
        headers = {'content-type': 'application/json'}
        response = send_etsy_post_request(url, data, headers=headers)
    ...
    def filter_receipts(self, receipts):
        ...
        for receipt in receipts:
            if receipt['country_id'] in (Etsy.CANADA_COUNTRY_ID, USA_COUNTRY_ID):
