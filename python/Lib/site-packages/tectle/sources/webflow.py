...
class Webflow(Source):
    ...
    def filter_receipts(self, receipts):
        ...
        for receipt in receipts:
            if receipt['shippingAddress']['country'] in self.countries:
                filtered_receipts.append(receipt)
            else:
                excluded_receipts.append(receipt)
            
        logger.debug('Excluded Webflow Receipts')
        logger.debug('\n'.join(['%s - %s' % (receipt['orderId'], receipt['shippingAddress']['country'])
                                    for receipt in excluded_receipts]))
        return filtered_receipts

    def format_receipt(self, receipt):
        data = {
            'name': receipt['shippingAddress']['addressee'].encode('latin-1', 'ignore').decode('latin-1', 'ignore'),
            ...
            'country': self.countries[receipt['shippingAddress']['country']]['country_code'],
            'country_id': self.countries[receipt['shippingAddress']['country']]['country_id'],
    ...
    def fetch_receipts_helper(self, offset):
        ...
        if not is_debug():
            ...
            url = 'http://%s:80/sites/orders?limit=100&offset=%s' % (get_local_ip(), offset)
    ...
    def fetch_receipt_items_helper(self, receipt):
        ...
        for purchased_items in receipt['purchasedItems']:
            sku = purchased_items['variantSKU']
