...
class Source(ABC):
    ...
    def _get_countries(self):
        # look up countries by id (209), code (CA), or name (Canada)
        conn = get_connection()
        cur = conn.cursor()
        query = "SELECT * FROM countries"
        rows = get_data_dict(cur, query)
        self.countries = {} # ISO code or Name
        self.country_ids = {} # Etsy Country Id
        for row in rows:
            self.country_ids[row['country_id']] = row
            self.countries[row['country_code']] = row
            self.countries[row['name']] = row
        cur.close()
        conn.close()
    ...
    def fetch_and_insert(self):
        ...
        # there could be duplicate in src_receipts so let's remove em
        # this can happen if a new order is added while we are fetching from the api
        unique_receipt_ids_set = set()
        unique_receipts = []
        for receipt in src_receipts:
            receipt_id = self.get_receipt_id(receipt)
            if receipt_id not in unique_receipt_ids_set:
                unique_receipt_ids_set.add(receipt_id)
                unique_receipts.append(receipt)
            else:
                logger.debug("DUPLICATE: %s" % receipt_id)
        unique_receipt_ids = list(unique_receipt_ids_set)
        logger.debug("SRC: %s" % ','.join(unique_receipt_ids))
        conn = get_connection()
        cur = conn.cursor()
        if not conn.in_transaction:
            cur.execute("BEGIN");
        new_receipt_ids = self.get_new_receipt_ids(cur, unique_receipt_ids)
        logger.debug("new: %s" % ','.join(new_receipt_ids))
        
        if len(new_receipt_ids) > 0:
            new_receipts = [receipt for receipt in unique_receipts 
            ...
            if len(parseable_receipt_ids) > 0:
                ...
                logger.debug("PARSEABLE: %s" % ','.join([receipt['receipt_id'] for receipt in parseable_receipts]))
    ...
    def insert_receipt_items(self, cur, receipt_items):
        ...
        receipt_ids = list(receipt_items.keys())
    ...
    def get_receipt_parseable_items(self, receipt_items):
        ...
        for receipt_id in receipt_items:
            ...
            for fullsku in fullskus:
                ...
                has_planter |= fullsku.startswith('P')
            if items and has_planter:
