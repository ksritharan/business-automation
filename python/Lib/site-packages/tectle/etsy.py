...
def check_etsy_or_redirect():
    ...
    if 'oauth_token' not in session or 'oauth_secret' not in session:
        ...
        if not config['oauth_token'] or not config['oauth_secret']:
            ...
            callback_uri = 'http://127.0.0.1:8080/etsy-callback'
...
def send_etsy_post_request(url, data=None, params=None, headers=None):
    if 'oauth_token' not in session or 'oauth_secret' not in session:
        conn = get_connection()
        cur = conn.cursor()
        query = """
    ...
    return oauth.post(url, data=data, params=params)