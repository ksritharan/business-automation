...
def delete_from_inventory(cur, receipts):
    ...
    query = """
        SELECT sku,
          ...
          FROM products
         WHERE sku in ('%s')
    """ % "','".join(list(counts.keys()))
    rows = get_data_dict(cur, query)
    sku_info = {row['sku']:{'color': row['color'], 'weight': row['filament_weight_kg']} for row in rows}
    filament_update = {}
    ...
    for sku in counts:
        ...
        cur.execute(query)
        color = sku_info[sku]['color']
        weight = sku_info[sku]['weight']
        if color not in filament_update:
            filament_update[color] = 0
        filament_update[color] += weight * counts[sku]
    
    for color in filament_update:
        filament_update[color] = '%.3f' % filament_update[color]
    
    # update filament weights
    template_query = """
        UPDATE filament_inventory
           SET weight_kg = MAX(0, weight_kg - %s)
         WHERE color = '%s'
    """
    for color in filament_update:
        query = template_query % (filament_update[color], color)
        cur.execute(query)
    ...
    query = """
        SELECT box_id,
         ...
         GROUP BY box_id;
    """ % "','".join([receipt['receipt_id'] for receipt in receipts])
    rows = get_data_dict(cur, query)
    template_query = """
        UPDATE boxes
           SET quantity = MAX(0, quantity - %(count)s)
         WHERE id = %(box_id)s
    """
    for row in rows:
        query = template_query % row
        cur.execute(query)
...
def get_new_group_id(cur):
    ...
    cur.execute(query)
    num = cur.fetchone()[0]
    stripped = date_key.replace('-','')
    group_id = '84678476%s%s' % (stripped, num+1)
    query = """
...
def get_all_receipts(cur):
    ...
    for receipt in receipts:
        receipt['order_info_dict'] = eval(receipt['order_info'])
        receipt['queue_items_dict'] = eval(receipt['queue_items'])
        receipt['order_date_str'] = datetime.fromtimestamp(receipt['order_time']).strftime('%Y-%m-%d %H:%M:%S')
    return get_oldest_helper(inventory, receipts)
    
def get_oldest_helper(inventory, receipts):
    fulfilled_receipts = []
    unfulfilled_receipts = []
    if receipts:
        for receipt in receipts:
            satisfied = greedy_remove_from_inventory(inventory, receipt['order_info_dict'], receipt['queue_items_dict'])
            if satisfied:
                fulfilled_receipts.append(receipt)
            else:
                unfulfilled_receipts.append(receipt)
    return fulfilled_receipts, unfulfilled_receipts

def greedy_remove_from_inventory(inventory, order_info, queue_items):
    satisfied = True
    for sku in order_info:
        if sku in inventory:
            if inventory.get(sku, 0) < order_info[sku]:
                satisfied = False
            min_amt = min(inventory.get(sku, 0), order_info[sku])
            inventory[sku] -= min_amt
            order_info[sku] -= min_amt
            queue_items[sku] = queue_items[sku][min_amt:]
        else:
            satisfied = False
